...
## Summary
The Medical Prediction Project is designed to assist healthcare providers, especially in regions with limited resources, in predicting Medical based on selected symptoms. This project includes two main files:
- **Medical_prediction.py**: The core app where users input symptoms, and the model predicts possible Medical.
...
Artificial Intelligence (AI) and Machine Learning (ML) are integral to this project. By training an ML model on a dataset of symptoms and Medical, the app can provide quick Medical predictions based on user input, allowing healthcare providers to make faster and more informed decisions.

## File Breakdown

### `Medical_prediction.py`
- **Purpose**: This file is the main driver for the web-based app that allows users to select symptoms and get predictions for potential Medical using a trained machine learning model.
- **Role of AI/ML**: The app loads a pre-trained MLP model that processes the symptoms inputted by the user and returns predictions for the most likely Medical.
...
### `main.ipynb`
- **Purpose**: This notebook handles training the machine learning models. It explores different neural network architectures to determine the best performer for predicting Medical based on symptoms.
...
1. **Pandas**:
   ...
   - Handles missing data and encodes categorical variables (e.g., Medical and symptoms).

2. **Seaborn & Matplotlib**:
   - Visualizes the dataset during EDA to understand trends, distributions, and relationships between symptoms and Medical.
...
1. **Multi-Layer Perceptron (MLP)**:
   - The MLP model was implemented using Keras's `Sequential` API. It consists of fully connected layers that process symptoms as input and predict the likelihood of different Medical.
